...
pub fn validate_ascii(c: &mut Criterion) {
    ...
    c.bench_function("Ascii::validate", |b| b.iter(|| {
        Ascii::validate(black_box(&data)).unwrap()
    }));
}
...
pub fn validate_utf16(c: &mut Criterion) {
    ...
    c.bench_function("Utf16::validate", |b| b.iter(|| {
        Utf16::validate(black_box(&data)).unwrap()
    }));
}
